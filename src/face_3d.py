"""
note: 
- face3d_468 and face3d_68 are totally different
- face3d_14 is a subset of face3d_68
...
def get_landmark_index(name):
    if name == "3d:14/68":
        return [33, 29, 34, 38, 13, 17, 25, 21, 55, 49, 43, 39, 45, 6]
    elif name == "2d:14/68":
        return [17, 21, 22, 26, 36, 39, 42, 45, 31, 35, 48, 54, 57, 8]
    elif name == "vino:14/35":
        return [12, 14, 15, 17, 1, 0, 2, 3, 6, 7, 8, 9, 11, 26]
    else:
...
def get_cube_pts():
    ...
    ymin = object_pts[:, 1].min()
    zmax = object_pts[:, 2].max()
    zmin = object_pts[:, 2].min()
    # get largest side
    xlen = xmax - xmin
    ylen = ymax - ymin
    zlen = zmax - zmin
...
def get_face3d(index=None):
    ...
    if index is None:
        ...
        # index is list
        return object_pts[index, :]


def get_face3d_468(index=None):
    """
    # face model from mediapipe
    ...
    #object_pts -= object_pts[1]
    #object_pts *= np.array([1, -1, -1])  # fix axis
    if index is None:
        return object_pts
    else:
        # index is list
...
if __name__ == "__main__":
    object_pts = get_face3d_468()
    print("face3d_468 has shape: {}".format(object_pts.shape))
    ...
    landmark_idx = []
    for pt in object_14_pts:
        for idx, pt_68 in enumerate(object_pts):
            if (pt == pt_68).all():
                landmark_idx.append(idx)
    print("14pts in 68pts has index: {}".format(landmark_idx))

    #landmarks_idx = 
    #object_pts = get_face3d_468(landmarks_idx)
    #print("selected face3d_68 has shape: {}".format(object_pts.shape))